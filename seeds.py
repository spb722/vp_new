...
extra_seeds = [
    ...
    make_seed(
        seed_id="S158_last_n_months_max_bounded",
        description="Generic MAX KPI over last N completed months using bounded CurrentMonth window",
        client="both",
        output_template="{date_col} >= CurrentMonth-{N}MONTHS AND {date_col} < CurrentMonth AND MAX({kpi_col}) ${operator} ${value}",
        agg_type="MAX",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="bounded",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports bounded month-window MAX patterns such as maximum data usage over the last 3 months."
    ),
    make_seed(
    ...
    make_seed(
        ...
        description="Raw precomputed KPI comparison without an explicit time window",
        client="both",
        output_template="{kpi_col} ${operator} ${value}",
        agg_type="RAW",
        reasoning=(
            "Supports customer-360/precomputed metrics where the KPI column already "
            "encodes the business window, such as M1, MTD, 15D, 60D, W4, or 90D count."
        ),
    ),
    make_seed(
        seed_id="S162_raw_kpi_days_lower_only",
        description="Raw KPI comparison over a rolling day window",
        client="both",
        output_template="{date_col} >= CurrentTime-{N}DAYS AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["DAYS"],
        anchors=["CurrentTime"],
        bound_style="lower_only",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports non-aggregated KPI comparisons where the API-resolved KPI should be thresholded inside a rolling day window.",
    ),
    make_seed(
        seed_id="S163_raw_kpi_weeks_lower_only",
        description="Raw KPI comparison over a rolling week window",
        client="both",
        output_template="{date_col} >= CurrentWeek-{N}WEEKS AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["WEEKS"],
        anchors=["CurrentWeek"],
        bound_style="lower_only",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports non-aggregated KPI comparisons over rolling weeks.",
    ),
    make_seed(
        seed_id="S164_raw_kpi_months_lower_only",
        description="Raw KPI comparison over a rolling month window",
        client="both",
        output_template="{date_col} >= CurrentMonth-{N}MONTHS AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="lower_only",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports non-aggregated KPI comparisons over rolling months.",
    ),
    make_seed(
        seed_id="S165_raw_kpi_month_exact",
        description="Raw KPI comparison for an exact previous month",
        client="both",
        output_template="{date_col} = CurrentMonth-{N}MONTHS AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["MONTHS"],
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports non-aggregated KPI comparisons for a pinned previous month.",
    ),
    make_seed(
        seed_id="S166_last_n_weeks_avg_lower_only",
        description="Generic AVG KPI over last N weeks using lower-only CurrentWeek window",
        client="both",
        output_template="{date_col} >= CurrentWeek-{N}WEEKS AND AVG({kpi_col}) ${operator} ${value}",
        agg_type="AVG",
        units=["WEEKS"],
        anchors=["CurrentWeek"],
        bound_style="lower_only",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports direct AVG over week windows when the user asks for an average value rather than an average-over-period formula.",
    ),
    make_seed(
        seed_id="S167_avg_formula_months_bounded",
        description="Average KPI over completed last N months using virtual formula and bounded CurrentMonth window",
        client="both",
        output_template="{date_col} >= CurrentMonth-{N}MONTHS AND {date_col} < CurrentMonth+0MONTHS AND SUM(V{{{vp_name}}}=f{{{kpi_col}/{divisor}}}) ${operator} ${value}",
        agg_type="FORMULA",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="bounded",
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports average monthly KPI requests over completed month windows.",
    ),
    make_seed(
        seed_id="S168_raw_kpi_days_bounded",
        description="Raw KPI comparison over a completed rolling day window",
        client="both",
        output_template="{date_col} >= CurrentTime-{N}DAYS AND {date_col} < CurrentTime AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["DAYS"],
        anchors=["CurrentTime"],
        bound_style="bounded",
        requires_date_col=True,
        requires_N=True,
        reasoning="Adds a strict RAW bounded day-window shape for precomputed KPI comparisons that still require an explicit completed day range.",
    ),
    make_seed(
        seed_id="S169_raw_kpi_weeks_bounded",
        description="Raw KPI comparison over completed rolling weeks",
        client="both",
        output_template="{date_col} >= CurrentWeek-{N}WEEKS AND {date_col} < CurrentWeek AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["WEEKS"],
        anchors=["CurrentWeek"],
        bound_style="bounded",
        requires_date_col=True,
        requires_N=True,
        reasoning="Adds a strict RAW bounded week-window shape for completed-week comparisons.",
    ),
    make_seed(
        seed_id="S170_raw_kpi_months_bounded",
        description="Raw KPI comparison over completed rolling months",
        client="both",
        output_template="{date_col} >= CurrentMonth-{N}MONTHS AND {date_col} < CurrentMonth AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="bounded",
        requires_date_col=True,
        requires_N=True,
        reasoning="Adds a strict RAW bounded month-window shape for completed-month comparisons.",
    ),
    make_seed(
        seed_id="S171_count_all_days_bounded",
        description="Generic COUNT_ALL over completed rolling days",
        client="both",
        output_template="{date_col} >= CurrentTime-{N}DAYS AND {date_col} < CurrentTime AND COUNT_ALL({count_col}) ${operator} ${value}",
        agg_type="COUNT_ALL",
        units=["DAYS"],
        anchors=["CurrentTime"],
        bound_style="bounded",
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Generic bounded day-window count seed for completed-day count requests.",
    ),
    make_seed(
        seed_id="S172_count_all_weeks_lower_only",
        description="Generic COUNT_ALL over last N weeks",
        client="both",
        output_template="{date_col} >= CurrentWeek-{N}WEEKS AND COUNT_ALL({count_col}) ${operator} ${value}",
        agg_type="COUNT_ALL",
        units=["WEEKS"],
        anchors=["CurrentWeek"],
        bound_style="lower_only",
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Generic rolling-week count seed without presence/absence semantics.",
    ),
    make_seed(
        seed_id="S173_count_all_months_exact",
        description="Generic COUNT_ALL for an exact previous month",
        client="both",
        output_template="{date_col} = CurrentMonth-{N}MONTHS AND COUNT_ALL({count_col}) ${operator} ${value}",
        agg_type="COUNT_ALL",
        units=["MONTHS"],
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Generic exact-month count seed for pinned month count requests.",
    ),
    make_seed(
        seed_id="S174_count_all_months_bounded",
        description="Generic COUNT_ALL over completed rolling months",
        client="both",
        output_template="{date_col} >= CurrentMonth-{N}MONTHS AND {date_col} < CurrentMonth AND COUNT_ALL({count_col}) ${operator} ${value}",
        agg_type="COUNT_ALL",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="bounded",
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Generic bounded month-window count seed for completed-month count requests.",
    ),
    make_seed(
        seed_id="S175_avg_days_lower_only",
        description="Generic AVG KPI over last N days",
        client="both",
        output_template="{date_col} >= CurrentTime-{N}DAYS AND AVG({kpi_col}) ${operator} ${value}",
        agg_type="AVG",
        units=["DAYS"],
        anchors=["CurrentTime"],
        bound_style="lower_only",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports direct AVG over rolling days when no average-over-period formula is intended.",
    ),
    make_seed(
        seed_id="S176_avg_months_lower_only",
        description="Generic AVG KPI over last N months",
        client="both",
        output_template="{date_col} >= CurrentMonth-{N}MONTHS AND AVG({kpi_col}) ${operator} ${value}",
        agg_type="AVG",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="lower_only",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports direct AVG over rolling months when no average-over-period formula is intended.",
    ),
    make_seed(
        seed_id="S177_percentage_formula_days_lower_only",
        description="Calculated percentage of a KPI over last N days using virtual formula",
        client="both",
        output_template="{date_col} >= CurrentTime-{N}DAYS AND SUM(V{{{vp_name}}}=f{{({kpi_col}*{factor})}}) ${operator} ${value}",
        agg_type="FORMULA",
        units=["DAYS"],
        anchors=["CurrentTime"],
        bound_style="lower_only",
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Adds a strict rolling-day percentage formula seed for percentage-of-KPI requests with explicit day windows.",
    ),
    make_seed(
        seed_id="S178_raw_kpi_month_mtd",
        description="Raw KPI comparison for current month-to-date",
        client="both",
        output_template="{date_col} >= CurrentMonth-0MONTHS AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["MONTHS"],
        anchors=["CurrentMonth"],
        bound_style="lmtd",
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports RAW KPI requests with current month till date / MTD wording without dropping the explicit time bound.",
    ),
    make_seed(
        seed_id="S179_raw_kpi_week_exact",
        description="Raw KPI comparison for an exact previous week index",
        client="both",
        output_template="{date_col} = CurrentWeek-{N}WEEKS AND {kpi_col} ${operator} ${value}",
        agg_type="RAW",
        units=["WEEKS"],
        ...
        requires_date_col=True,
        requires_N=True,
        reasoning="Supports phrases like week 6 where the request refers to a specific week offset rather than a rolling week range.",
    ),
